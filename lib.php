...
function bootstrap_creation_require_course_login($courseorid, $autologinguest = true, $cm = NULL, $setwantsurltome = true, $preventredirect = true) {
    global $CFG, $SITE;
    ...
    if (!empty($CFG->forcelogin)) {
        ...
        bootstrap_creation_require_login($courseorid, $autologinguest, $cm, $setwantsurltome, $preventredirect);
...
function bootstrap_creation_require_login($courseorid = NULL, $autologinguest = true, $cm = NULL, $setwantsurltome = true, $preventredirect = false) {
...
class bootstrap_creation_expand_navigation extends global_navigation {
    ...
    public function expand($branchtype, $id) {
        global $CFG, $DB, $PAGE;
        static $bootstrap_creation_expanded_courses = array();
        ...
        switch ($branchtype) {
            ...
            case self::TYPE_COURSE :
                ...
                try {
                    if(!array_key_exists($course->id, $bootstrap_creation_expanded_courses)) {
                        ...
                        if ($PAGE->course->id == $course->id && (!method_exists($this, 'format_display_course_content') || $this->format_display_course_content($course->format))) {
                            bootstrap_creation_require_course_login($course);
                            $bootstrap_creation_expanded_courses[$course->id] = $this->load_course_sections($course, $coursenode);
                        }
                    }
            ...
            case self::TYPE_SECTION :
                ...
                try {
                    ...
                    if (empty($bootstrap_creation_expanded_courses[$course->id])) {
                        ...
                        $bootstrap_creation_expanded_courses[$course->id] = $sectionnodes;
                    }
                    $sections = $bootstrap_creation_expanded_courses[$course->id];
...
class bootstrap_creation_dummy_page extends moodle_page {
    /**
 ...
 * @return string
 */
function bootstrap_creation_user_settings($css, $theme) {

    // Set the page Site color
    if (empty($theme->settings->sitebgc)) {
        $sitebgc = '#7AB80E'; // default 
        $sitegrd = "#7AB80E";
    } else {
        $sitebgc = $theme->settings->sitebgc;
        

    }
     $css = bootstrap_creation_set_sitebgc($css, $sitebgc);
     return $css;
 ...
 *
 */
function bootstrap_creation_set_sitebgc($css, $sitebgc) {
	
    $tag = '[[setting:sitebgc]]';
    $css = str_replace($tag, $sitebgc, $css);
...
function bootstrap_creation_mk_height_asbar($asbheight, $css){
...
function bootstrap_creation_get_js_module() {
    global $PAGE;
    return array(
        'name' => 'theme_bootstrap_creation',
        'fullpath' => '/theme/bootstrap_creation/yui/slider.js',
